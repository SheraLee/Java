...
// Extended by: Xirui Li (CS12fhm,A13691309)
...
class Unicalc
{
    ...
    /**
     * Starting one Check the type of operation
    ...
    public AST S()
    {
        ...
        if(next.equals("def")){
        	toks.pop();
        	String laterUnit = toks.peek();
        	if(laterUnit == null){
        		return null;
        	}
        	toks.pop();
        	AST newResult = new Define(laterUnit,L());
            return newResult;
    ...
    /**
     * to noramlize or pass on
     * @return result AST after L()
     */
    public AST L()
    {
        // L -> # E | E
    ...
    public AST E()
    {
        ...
        if ("+".equals(toks.peek())) {
            toks.pop();
            AST newResult = new Sum(p, E());
            return newResult;
        }
        else if ("-".equals(toks.peek())) {
            toks.pop();
            AST newResult = new Difference(p, E());
            return newResult;
        }
        else {
            return p;
    ...
    public AST Q()
    {
        ...
        if ( Unicalc.isAlphabetic(next) || Unicalc.isNumber(next) || next.equals("(")){
        	AST newResult = new Product(r,Q());
        	return newResult;
        }
    ...
    /**
     * R -> V | V ^ J
     * First start with R() can handle strings derivable from R in the grammar
     * @return the AST after Power level
     */
    public AST R()
    {
        // R -> V | V ^ J
        ...
        if ( next == null ) {
          ...
          return v;  // I don't think we should *always* do this
        }
    }
